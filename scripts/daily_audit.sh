...
FEED_URL="https://localhost:8443"
# The app is only reachable through Caddy (tls internal); curl reads
# CURL_CA_BUNDLE as --cacert, so every call below trusts the local root.
...
POSTS_TOTAL=$(run_sql "SELECT COUNT(*) FROM works WHERE deleted_at IS NULL;")
POSTS_NEW=$(run_sql "SELECT COUNT(*) FROM works WHERE deleted_at IS NULL AND created_at > NOW() - INTERVAL '24 hours';")
FOLLOWS_NEW=$(run_sql "SELECT COUNT(*) FROM follows WHERE created_at > NOW() - INTERVAL '24 hours';")
LIKES_NEW=$(run_sql "SELECT COUNT(*) FROM work_reactions WHERE reaction_type = 'like' AND created_at > NOW() - INTERVAL '24 hours';")
...
# Gnosis (messaging lives in feed-engine, f33d3r_feed) — sealed messages awaiting delivery
MSG_PENDING=$(run_sql "SELECT COUNT(*) FROM gnosis_pending_messages;")
...
ONLINE_COUNT=0
OFFLINE_LIST=""
for brain in "${BRAIN_NANTAR}" "${BRAIN_AETHYRRANK}" "${BRAIN_AINSOPH}" "${BRAIN_ELOHIM}" "${BRAIN_ZODACARE}" "${BRAIN_VERITY}" "${BRAIN_LEDGER}"; do
  name="${brain%%:*}"
  status="${brain##*:}"
  if [[ "${status}" == "ok" ]]; then
