/// Bot heuristic detection based on account age and posting volume.
use chrono::Utc;
...
pub async fn detect(pool: &Pool, pial_id: &str) -> BotResult {
    let Some((created_at, post_count)) =
        crate::db::get_account_age_and_post_count(pool, pial_id).await
    else {
        return BotResult {
    ...
    // New account burst: <1 day old AND >5 posts
    if age_hours < 24 && post_count > 5 {
        signals.push("bot_new_account_burst".to_string());
        is_suspected_bot = true;
    }
    // New account high volume: <3 days old AND >15 posts
    if age_days < 3 && post_count > 15 {
