...
fn validate_handle(handle: &str) -> Result<(), (StatusCode, Json<Value>)> {
    if handle.is_empty() || handle.len() > 30 {
        return Err(err(StatusCode::BAD_REQUEST, "handle must be 1–30 characters"));
    }
    if !handle.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
        return Err(err(StatusCode::BAD_REQUEST, "handle may only contain a-z A-Z 0-9 _"));
    }
    Ok(())
}
...
async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "service": "registry-brain" }))
}
...
async fn register_handle(
    ...
    db::log_event(&state.pool, row.id, "register", None, Some(req.pial_id), json!({ "tier": tier })).await;
...
async fn bind_handle(
    ...
    db::log_event(&state.pool, row.id, "bind", None, Some(req.pial_id), json!({ "requester": req.requester })).await;
    Ok(Json(to_handle_resp(row)))
}
...
async fn unbind_handle(
    ...
    .await
    .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()))?
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "handle not found or not owned by requester"))?;

    db::log_event(&state.pool, row.id, "unbind", Some(req.requester), None, json!({})).await;
    Ok(Json(to_handle_resp(row)))
}
...
async fn transfer_handle(
    ...
    .await
    .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()))?
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "handle not found or not owned by from_pial"))?;

    db::log_event(
        &state.pool, row.id, "transfer",
        Some(req.from_pial), Some(req.to_pial),
...
async fn admin_action(
    ...
    let new_status = match req.action.as_str() {
        ...
        "reclaim"    => "reserved",
        _ => return Err(err(StatusCode::BAD_REQUEST, "unknown action: freeze | unfreeze | quarantine | reclaim")),
    };
    ...
    db::log_event(
        &state.pool, row.id, &req.action, None, None,
...
async fn create_auction(
    ...
    db::log_event(
        &state.pool, hrow.id, "auction_start", Some(req.requester), None,
...
async fn place_bid(
    ...
    if req.amount_aet <= arow.current_price_aet {
        return Err(err(StatusCode::BAD_REQUEST, "bid must exceed current price"));
    }

    sqlx::query("INSERT INTO auction_bids (auction_id, bidder_pial, amount_aet) VALUES ($1, $2, $3)")
        .bind(auction_id)
    ...
    let bid_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM auction_bids WHERE auction_id = $1")
        .bind(auction_id)
        .fetch_one(&state.pool)
        .await
        .unwrap_or(0);
...
async fn settle_auction(
    ...
    if arow.status != "active" {
        return Err(err(StatusCode::CONFLICT, "auction already settled or cancelled"));
    }

    let new_status = if arow.highest_bidder_pial.is_some() { "settled" } else { "ended" };
    ...
    if let Some(winner) = arow.highest_bidder_pial {
        ...
        db::log_event(
            &state.pool, arow.handle_id, "auction_settle", None, Some(winner),
