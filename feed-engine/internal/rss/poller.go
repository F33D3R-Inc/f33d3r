...
var feeds = []feedDef{
	...
	// No AP News: feeds.apnews.com is NXDOMAIN and apnews.com/index.rss needs a
	// licensed AP Newsroom key. Do not add a guessed AP URL.
	// Tech
	...
	{"https://sports.yahoo.com/nfl/rss/", "Yahoo NFL", "sports"},
	{"https://www.cbssports.com/rss/headlines/nfl/", "CBS NFL", "sports"},
	{"https://profootballtalk.nbcsports.com/feed/", "Pro Football Talk", "sports"},
	// Finance
	...
	for _, f := range feeds {
		...
		go func() {
			...
			if err != nil {
				...
				ch <- result{category: f.Category}
				return
			}
			ch <- result{category: f.Category, items: items, ok: true}
		}()
	}

	byCategory := make(map[string][]Item)
	// fetched marks categories where at least one feed answered, separating
	// "all feeds failed" from "genuinely empty".
	fetched := make(map[string]bool)
	for range feeds {
		r := <-ch
		byCategory[r.category] = append(byCategory[r.category], r.items...)
		if r.ok {
			fetched[r.category] = true
		}
	}

	c.mu.Lock()
	for cat, items := range byCategory {
		// All feeds failed: keep the last good items rather than blanking the rail.
		if !fetched[cat] {
			continue
		}
