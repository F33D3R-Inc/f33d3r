...
func MigrateUp(database *sql.DB) {
	...
	// Additive column migrations — idempotent, safe on every boot.
	additive := []string{
		...
		// Moderation dedup — collapse any pre-existing duplicate pending reports
		// (same reporter on the same content) down to the earliest row, then add a
		// partial-unique index so a reporter can hold only one open report per item.
		// This kills the "triple" cards at the source.
	...
	for _, stmt := range additive {
		if _, err := database.Exec(stmt); err != nil {
			log.Fatalf("[db] additive migration failed: %v", err)
		}
	}
