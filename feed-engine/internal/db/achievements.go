...
func countQuery(database *sql.DB, q string, args ...interface{}) int {
	var n int
	database.QueryRow(q, args...).Scan(&n)
	return n
}
...
func TryAwardTrollOnPost(database *sql.DB, userID, pialID, postID, body string) {
	...
	lateNight := countQuery(database,
		`SELECT COUNT(*) FROM works WHERE author_pial_id = $1
	...
	hashtagPosts := countQuery(database,
		`SELECT COUNT(*) FROM works WHERE author_pial_id = $1 AND body LIKE '%#%' AND deleted_at IS NULL`, pialID)
	if hashtagPosts >= 50 {
	...
	botPosts := countQuery(database,
		`SELECT COUNT(*) FROM works WHERE author_pial_id = $1 AND body ILIKE '%bot%' AND deleted_at IS NULL`, pialID)
	if botPosts >= 100 {
	...
	replyOtherThreads := countQuery(database,
		`SELECT COUNT(*) FROM works w
		 ...
		 WHERE w.author_pial_id = $1 AND w.kind = 'reply'
		   AND (root.id IS NULL OR root.author_pial_id != $1)
	...
	maxRepliesInThread := countQuery(database,
		`SELECT COALESCE(MAX(rc),0) FROM (
			...
			WHERE author_pial_id = $1 AND kind = 'reply' AND deleted_at IS NULL
			GROUP BY root_cid
		) t`, pialID)
...
func TryAwardTrollOnReplyReceived(database *sql.DB, authorID, authorPIAL, postID, replyBody string) {
	...
	uniqueRepliers := countQuery(database,
		`SELECT COUNT(DISTINCT w.author_pial_id)
		 FROM works w
		 JOIN works parent ON parent.cid = w.parent_cid
		 WHERE parent.author_pial_id = $1
		   AND w.author_pial_id != $1
	...
	newEngagers := countQuery(database,
		`SELECT COUNT(DISTINCT w.author_pial_id)
		 ...
		 WHERE w.kind = 'reply' AND w.parent_id = $1::uuid
		   AND w.author_pial_id != $2
		   AND w.deleted_at IS NULL
		   AND w.author_pial_id NOT IN (
			   SELECT DISTINCT w2.author_pial_id FROM works w2
			   WHERE w2.kind = 'reply'
			     AND w2.parent_id IN (
				     SELECT id FROM works WHERE author_pial_id = $2 AND id != $1::uuid
			     )
...
func TryAwardTrollOnPostStats(database *sql.DB, authorID, authorPIAL, postID string) {
	...
	totalReposts := countQuery(database,
		`SELECT COUNT(*) FROM work_reactions wr
		 JOIN works w ON w.id = wr.work_id
		 WHERE w.author_pial_id = $1 AND wr.reaction_type = 'repost'`, authorPIAL)
	if totalReposts >= 500 {
...
func TryAwardTrollOnReported(database *sql.DB, reportedUserID string) {
	...
	// Blacklist Material — 10+ posts removed (is_blocked)
	...
	// Blacklist Material (legacy posts table too)
	removedPostsLegacy := countQuery(database,
		`SELECT COUNT(*) FROM posts WHERE author_id = $1 AND is_blocked = TRUE`, reportedUserID)
	if removedPosts+removedPostsLegacy >= 10 {
		AwardAchievement(database, reportedUserID, "troll_blacklist_material")
	}
...
func TryAwardTrollForumDemon(database *sql.DB, userID, pialID string) {
	distinctThreadsToday := countQuery(database,
		`SELECT COUNT(DISTINCT root_cid) FROM works
		 WHERE author_pial_id = $1 AND kind = 'reply'
...
func TryAwardTrollCartmanMethod(database *sql.DB, userID, authorPIAL, postID string) {
	uniqueParticipants := countQuery(database,
		`SELECT COUNT(DISTINCT author_pial_id) FROM works
		 WHERE root_cid = (SELECT cid FROM works WHERE id = $1::uuid)
		   AND author_pial_id != $2
...
// CountUserVisions returns the number of vision posts (kind='vision') by a user.
func CountUserVisions(database *sql.DB, userID string) int {
	var n int
	database.QueryRow(
		`SELECT COUNT(*) FROM works WHERE author_id = $1 AND kind = 'vision' AND deleted_at IS NULL`,
		userID,
	).Scan(&n)
	return n
}

// TryAwardVisionAchievements checks vision posting milestones and awards achievements.
// Call this after every successful vision post insertion.
func TryAwardVisionAchievements(database *sql.DB, userID string) {
	n := CountUserVisions(database, userID)
	if n == 1 {
