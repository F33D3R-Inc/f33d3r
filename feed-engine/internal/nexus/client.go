...
type Client struct {
	...
	// InternalKey is the service-to-service shared secret. Verity's NEXUS
	// mutations (identity creation, persona linking) are service-only — feed-
	// engine authenticates the user and derives the shard, then calls in as a
	// peer. Every mutating call carries this on X-Internal-Key.
	InternalKey string
}
...
// postJSON issues a POST carrying the service credential. The mutating NEXUS
// routes reject a request that does not hold it, so it goes on every one of them
// in a single place rather than being re-set (or forgotten) per call site.
...
func (c *Client) Verify(req VerifyRequest) (VerifyResponse, error) {
	...
	resp, err := c.postJSON("/v1/nexus/verify", body)
	if err != nil {
...
func (c *Client) ConfirmLink(requestID, pialShard string) error {
	...
	resp, err := c.postJSON("/v1/nexus/link/confirm", body)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func (c *Client) DeclineLink(requestID, pialShard string) error {
	...
	resp, err := c.postJSON("/v1/nexus/link/decline", body)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func (c *Client) InitiateLink(sourceShard, targetShard string) (InitiateLinkResponse, error) {
	...
	resp, err := c.postJSON("/v1/nexus/link/initiate", body)
	if err != nil {
