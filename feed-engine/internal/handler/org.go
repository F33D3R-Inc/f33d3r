...
// orgPendingInvitesTarget is the id of the org panel's "Sent invites" Facet.
// An org_invite submitted from the panel names it in HX-Target, and is answered
// with that Facet re-rendered; an invite sent from the settings section keeps
// its settings_section_org answer.
const orgPendingInvitesTarget = "org-pending-invites"

// renderOrgPendingInvites writes the org_pending_invites Facet for user plus an
// out-of-band re-render of the invite form, so the handle field is cleared by
// the server's answer and not by page script.
func (h *Handler) renderOrgPendingInvites(w http.ResponseWriter, user *model.User) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Header().Set("Cache-Control", "no-store")
	h.renderPartial(w, "org_pending_invites", map[string]interface{}{
		"PendingInvites": dbpkg.GetOrgPendingInvites(h.db, user.ID),
	})
	h.renderPartial(w, "org_invite_form", map[string]interface{}{"OOB": true})
}
...
func (h *Handler) orgEvent(w http.ResponseWriter, r *http.Request, eventType string) {
	...
	case "org_invite":
		...
		if r.Header.Get("HX-Target") == orgPendingInvitesTarget {
			h.renderOrgPendingInvites(w, user)
			return
		}
		h.renderOrgSection(w, r)
...
func (h *Handler) orgPanelPage(w http.ResponseWriter, r *http.Request) {
	...
	h.render(w, r, "org_panel.html", data)
}
