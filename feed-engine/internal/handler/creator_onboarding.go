...
func (h *Handler) creatorWelcomePage(w http.ResponseWriter, r *http.Request) {
	...
	h.render(w, "creator_welcome.html", map[string]interface{}{
		"User":  user,
...
func (h *Handler) creatorSetupPage(w http.ResponseWriter, r *http.Request) {
	...
	// Redirect preserves return path so after KYC the user lands back here.
	if !user.IsVerified && !user.IsAdmin() {
		http.Redirect(w, r, "/kyc?return_to=/create/setup", http.StatusSeeOther)
		return
	}

	h.render(w, "creator_setup.html", map[string]interface{}{
		"User":  user,
...
func (h *Handler) handleCreatorSetupComplete(w http.ResponseWriter, r *http.Request, rawBody map[string]json.RawMessage) {
	...
	if h.db != nil {
		...
		_ = dbpkg.SetCreatorMode(h.db, user.ID, true)
		...
		// Mark as adult creator in user_profiles if flag set.
		if isAdult {
			_, _ = h.db.Exec(
