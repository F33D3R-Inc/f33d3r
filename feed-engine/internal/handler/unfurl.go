...
// NOTE: triggerLinkPreview (the posts twin of the function below) was removed
// with the posts lane. It wrote a post_link_previews row joining a post to a URL;
// a work needs no such row, because EnrichWorksWithLinkPreviews resolves the
// preview from link_previews by the URL it extracts from the body itself. One
// cache, keyed by the thing it caches.
