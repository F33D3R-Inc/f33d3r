...
func (h *Handler) termsPage(w http.ResponseWriter, r *http.Request) {
	...
	h.render(w, "terms.html", map[string]interface{}{
		"User":      user,
...
func (h *Handler) dmcaPage(w http.ResponseWriter, r *http.Request) {
	user := h.userFromRequest(w, r)
	h.render(w, "dmca.html", map[string]interface{}{
		"User":      user,
...
func (h *Handler) requestDataDeletion(w http.ResponseWriter, r *http.Request) {
	...
	// Queue deletion — actual deletion is deferred so 2257 records can be retained
	_, err := h.db.Exec(`
...
func (h *Handler) requestDataExport(w http.ResponseWriter, r *http.Request) {
	...
	if h.db != nil {
		...
		h.db.QueryRow(`SELECT COUNT(*) FROM follows WHERE followed_id = $1`, user.ID).Scan(&followerCount)
		h.db.QueryRow(`SELECT COUNT(*) FROM follows WHERE follower_id = $1`, user.ID).Scan(&followingCount)
