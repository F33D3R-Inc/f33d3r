...
// requestIP extracts the real client IP. Uses X-Real-IP set by Caddy — never
// X-Forwarded-For, which clients can spoof by prepending arbitrary IPs.
...
func GetAdminAuditLog(db *sql.DB, limit int) []model.AdminAuditEntry {
	...
	rows, err := db.Query(`
		SELECT a.id, a.admin_id, COALESCE(u.handle,''), a.action, a.target_type, a.target_id, a.ip_address, a.created_at
...
func CheckBruteForce(db *sql.DB, ip string) {
	...
	if count > 5 {
		if !globalIPBlockCache.isBlocked(ip) {
			_ = BlockIP(db, ip, "brute force: >5 login failures in 5 minutes", "", true)
