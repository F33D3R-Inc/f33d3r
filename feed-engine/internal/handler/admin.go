...
// NOTE: POST /api/admin/migrate-works (adminMigrateWorks) was removed. It moved
// posts rows into works; once that was done it became a stub that answered
// {"ok":true} to a request that did nothing. The posts table it migrated from no
// longer exists, so the endpoint has nothing left to be a stub for.
...
func (h *Handler) adminSetRole(w http.ResponseWriter, r *http.Request) {
	...
	if h.db != nil && targetHandle != "" {
		...
		// The founder role carries a permanent realm floor (realm.FounderFloor), so a
		// role change moves standing. Restate it here rather than waiting for the
		// account's next XP award: promotion raises the ring immediately, and demotion
		// lets it fall back to exactly what the account has earned.
		if _, err := realm.Restate(h.db, target.ID); err != nil {
			log.Printf("[admin] realm restate after role change for @%s: %v", targetHandle, err)
		}
...
// realmStandingLabel renders, on the server, the one sentence the admin surface
// shows about an account's standing: what it wears, where that came from, and
// what it would fall back to if the floor were removed. The facet interpolates
// the result — it never compares a realm to a number or works out for itself
// whether standing was earned.
func realmStandingLabel(xp int64, grant int, role string) string {
	effective := realm.Effective(xp, grant, role)
	earned := realm.ComputeRealm(xp)
	worn := fmt.Sprintf("R%d %s", effective, realm.RealmName(effective))

	if !realm.IsGranted(xp, grant, role) {
		return fmt.Sprintf("%s — earned on %d XP", worn, xp)
	}
	source := fmt.Sprintf("a granted R%d floor", realm.Clamp(grant))
	if role == model.RoleFounder && realm.FounderFloor > realm.Clamp(grant) {
		source = fmt.Sprintf("the permanent R%d floor the founder role carries", realm.FounderFloor)
	}
	return fmt.Sprintf("%s — held by %s; earned R%d %s on %d XP",
		worn, source, earned, realm.RealmName(earned), xp)
}

// adminSetRealmGrant places an account on a granted realm floor, or clears one.
//
// Form fields:
//
//	target_handle — the account being placed
//	realm_grant   — "0" to clear, else "1".."5" (realm.Min..realm.Max)
//	reason        — required; it is what the ledger entry is for
//
// FOUNDER ONLY, deliberately stricter than the role control next to it.
// Role assignment is gated on IsAdmin() with founder restricted to one handle;
// a realm grant mints the platform's scarcest public signal — the ring every
// surface draws from the account's handle — so it is held at IsFounder(), the
// same gate as the treasury sweep and the hard purge. The card that posts here
// is likewise rendered for the founder alone, so an admin is never shown a
// control the server will refuse.
//
// The grant is a FLOOR (see package realm): it cannot be erased by a later XP
// award, it cannot cap what the account goes on to earn, and it cannot demote.
// Clearing it returns the account to exactly what its XP has earned.
func (h *Handler) adminSetRealmGrant(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "405", http.StatusMethodNotAllowed)
		return
	}
	...
	_ = r.ParseForm()
	targetHandle := strings.TrimSpace(r.FormValue("target_handle"))
	rawGrant := strings.TrimSpace(r.FormValue("realm_grant"))
	reason := strings.TrimSpace(r.FormValue("reason"))

	w.Header().Set("Content-Type", "text/html; charset=utf-8")

	if targetHandle == "" || rawGrant == "" {
		w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Missing fields.</span>`))
		return
	}
	// A grant is a privilege change on a public surface. It is never recorded
	// without a stated reason, because an unexplained one is exactly the entry
	// nobody can reconstruct six months later.
	if reason == "" {
		w.Write([]byte(`<span style="color:#ef4444;font-size:12px">A reason is required — it is what the audit entry says.</span>`))
		return
	}
	level, err := strconv.Atoi(rawGrant)
	if err != nil {
		w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Grant must be a realm level.</span>`))
		return
	}
	...
	// realm.SetGrant is the one writer of realm_grant and one of only two
	// writers of users.realm. It restates the standing in the same transaction
	// and republishes the account to the realm index, so the ring is correct on
	// the very next render — no restart, no reconcile wait.
	effective, err := realm.SetGrant(h.db, target.ID, level)
	if err != nil {
		log.Printf("[admin] realm grant R%d for @%s failed: %v", level, targetHandle, err)
		fmt.Fprintf(w, `<span style="color:#ef4444;font-size:12px">%s</span>`, template.HTMLEscapeString(err.Error()))
		return
	}

	// Audit trail: the append-only PIAL ledger, the same shape the capability
	// surface uses. Who granted it, to whom, what tier, when (created_at) and
	// why — a realm grant is never untraceable.
	event := "realm_granted"
	if level == realm.GrantNone {
		event = "realm_grant_cleared"
	}
	dbpkg.LogPIALEvent(h.db, target.PIALID, target.ID, event, map[string]interface{}{
		"grant":     level,
		"effective": effective,
		"earned":    realm.ComputeRealm(target.XP),
		"xp":        target.XP,
		"reason":    reason,
		"by":        caller.Handle,
	}, "admin")

	// Evict the target's cached session so the standing on their own next
	// request is the new one rather than lagging the 30s TTL.
	h.sessionCache.Delete("__handle__" + targetHandle)

	log.Printf("[admin] %s set realm_grant=%d on @%s → effective R%d (%s)",
		caller.Handle, level, targetHandle, effective, reason)

	if level == realm.GrantNone {
		fmt.Fprintf(w, `<span style="color:#22c55e;font-size:12px;font-weight:500">✓ Grant cleared for @%s — now R%d, earned</span>`,
			template.HTMLEscapeString(targetHandle), effective)
		return
	}
	fmt.Fprintf(w, `<span style="color:#22c55e;font-size:12px;font-weight:500">✓ @%s granted R%d — now wearing R%d (%s)</span>`,
		template.HTMLEscapeString(targetHandle), level, effective, realm.RealmName(effective))
}
...
func (h *Handler) resolveReport(w http.ResponseWriter, r *http.Request) {
	...
	if h.db != nil {
		...
		if _, err := dbpkg.ResolveReportsForContent(h.db, contentID, status, user.Handle); err != nil {
			log.Printf("[admin] reports NOT resolved for content=%s by @%s: %v", contentID, user.Handle, err)
			http.Error(w, "could not resolve reports", http.StatusInternalServerError)
			return
		}
		if err := dbpkg.ClearReportDrivenReview(h.db, contentID, user.Handle); err != nil {
			log.Printf("[admin] report-driven review NOT cleared for content=%s by @%s: %v", contentID, user.Handle, err)
			http.Error(w, "could not clear review hold", http.StatusInternalServerError)
			return
		}
	}
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
...
// fileContentReport is the one way a report is filed, whichever surface took
// it: the web report form (reportContent) and the `report` event the native
// clients send both end here. It records the report, logs it on the
// reporter's PIAL, moves adult/hate reports on works into human review and
// tells the admins, forwards to Zodacare, and checks the author's
// report-based achievements. The one failure that is the caller's to answer
// is the human-review transition: silently dropping it would accept the
// report, thank the reporter, and leave the content untouched.
func (h *Handler) fileContentReport(user *model.User, contentID, contentType, reason, detail string) error {
	if h.db != nil {
		...
		if contentType == "post" && (reason == "nsfw" || reason == "hate") {
			if err := dbpkg.SetPostScanState(h.db, contentID, "human_review", "user report: "+reason, "report_system"); err != nil {
				log.Printf("[admin] report on %s (%s): content NOT moved to human_review: %v",
					contentID, reason, err)
				return err
			}
...
func (h *Handler) adminInjectAET(w http.ResponseWriter, r *http.Request) {
	...
	// Ain Soph has no /v1/admin/credit route — that call has been 404-ing since it
	// was written, so admin AET injection never worked and the 404 was reported as
	// a generic "Ain-Soph error". The real mint path is /v1/deposit: it credits an
	// account named by a service caller (the acting identity comes from the caller,
	// the recipient from the body) and is the only route that brings AET into
	// existence. No new endpoint is invented here — the broken call is pointed at
	// the route that already does exactly this.
	//
	// Units: Ain Soph stores value as micro-AET (1 AET = 100 units); the admin form
	// takes whole AET. Deposit applies the standard 2.50% deposit fee, so the target
	// receives the net — the amount confirmed below reflects the request, and the
	// ledger records the fee split exactly as any other deposit.
	amountUnits := int64(math.Round(amount * 100))
	if amountUnits <= 0 {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Invalid amount.</span>`))
		return
	}
	// Idempotency: a double-submit of the same admin grant must not mint twice.
	// Keyed on actor + target + amount + reason + minute so a retry within the
	// window collapses, while a deliberate repeat later is its own grant.
	idem := fmt.Sprintf("admin_credit:%s:%s:%d:%s:%s",
		caller.Handle, target.PIALID, amountUnits, reason,
		time.Now().UTC().Format("2006-01-02T15:04"))
	payload, _ := json.Marshal(map[string]interface{}{
		"user_id":         target.PIALID,
		"amount_cents":    amountUnits,
		"idempotency_key": idem,
	})
	ctx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		h.cfg.AinSophURL+"/v1/deposit", bytes.NewReader(payload))
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	if err != nil {
	...
	req.Header.Set("Content-Type", "application/json")
	// Crediting somebody else's balance is a service act, not the admin acting as
	// themselves, so it authenticates as a peer.
	req.Header.Set("X-Internal-Key", h.cfg.InternalAPIKey)
	resp, err := h.httpClient.Do(req)
	if err != nil {
		log.Printf("[admin] AET inject transport error for @%s: %v", targetHandle, err)
		w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Ain-Soph unreachable — check logs.</span>`))
		return
	}
	defer resp.Body.Close()
	if resp.StatusCode >= 400 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 512))
		// A value path must never fail quietly. Surface the status and Ain Soph's
		// own error to the operator, not a generic string.
		log.Printf("[admin] AET inject failed for @%s: ain-soph %d: %s", targetHandle, resp.StatusCode, string(body))
		fmt.Fprintf(w, `<span style="color:#ef4444;font-size:12px">Ain-Soph rejected the credit (%d) — check logs.</span>`, resp.StatusCode)
		return
	}
	log.Printf("[admin] %s injected %.2f AET to @%s (%s)", caller.Handle, amount, targetHandle, reason)
	fmt.Fprintf(w, `<span style="color:#22c55e;font-size:12px;font-weight:500">✓ %.2f AET credited to @%s (net of deposit fee)</span>`, amount, targetHandle)
}
...
func (h *Handler) adminSweepFees(w http.ResponseWriter, r *http.Request) {
	...
	// The payer here is the platform fee float, not the admin making the click,
	// so this is a service acting for an account no session owns. Ain Soph now
	// derives the payer from the authenticated caller and only a peer holding the
	// shared secret may name a payer other than itself.
	xferReq.Header.Set("X-Internal-Key", h.cfg.InternalAPIKey)
...
func (h *Handler) adminUserAction(w http.ResponseWriter, r *http.Request) {
	...
	case "suspend":
		...
		if _, err := h.db.Exec(`
			...
			WHERE pial_id = $1`, target.PIALID); err != nil {
			log.Printf("[admin] user-action suspend: cooldown not set for @%s: %v", targetHandle, err)
			w.Write([]byte(`<span style="color:#ef4444;font-size:12px">DB error — check logs.</span>`))
			return
		}
		// The revocation IS the suspension. Reporting it as done when the write
		// failed leaves a suspended account still posting and a moderator who
		// believes otherwise.
		if err := dbpkg.SetCapability(h.db, target.PIALID, "posting", "revoked", reason, caller.Handle, nil); err != nil {
			log.Printf("[admin] user-action suspend: posting NOT revoked for @%s: %v", targetHandle, err)
			w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Posting was not revoked — check logs.</span>`))
			return
		}
		h.notifyUser(target.ID, "violation", caller.ID, "", "moderation")
	...
	case "ban":
		...
		// A ban that half-applied is the worst of both: the moderator is told the
		// account is gone and the account keeps whichever capability failed. Every
		// write is checked, and the first failure stops and says which one.
		if h.db != nil {
			if _, err := h.db.Exec(`UPDATE users SET role = 'banned' WHERE handle = $1`, targetHandle); err != nil {
				log.Printf("[admin] ban: role NOT set to banned for @%s: %v", targetHandle, err)
				w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Ban failed — check logs.</span>`))
				return
			}
		}
		_, nsfwTier, _ := dbpkg.GetTrustScore(h.db, target.PIALID)
		if err := dbpkg.SetTrustScore(h.db, target.PIALID, 0, nsfwTier, "terminated"); err != nil {
			log.Printf("[admin] ban: enforcement state NOT set to terminated for @%s: %v", targetHandle, err)
			w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Ban incomplete — check logs.</span>`))
			return
		}
		for _, cap := range []string{"posting", "messaging", "tipping", "subscribing", "live_streaming"} {
			if err := dbpkg.SetCapability(h.db, target.PIALID, cap, "revoked", reason, caller.Handle, nil); err != nil {
				log.Printf("[admin] ban: capability %q NOT revoked for @%s: %v", cap, targetHandle, err)
				fmt.Fprintf(w, `<span style="color:#ef4444;font-size:12px">Ban incomplete — %s was not revoked. Check logs.</span>`, cap)
				return
			}
		}
	...
	if h.db != nil {
		...
		if isAdultCreator {
			if role, _ := dbpkg.GetUserRole(h.db, target.ID); role == nil || !role.IsMinor {
				if err := dbpkg.SetContentSetting(h.db, target.ID, "adult_enabled"); err != nil {
					log.Printf("[admin] adult-creator grant for @%s: content_setting NOT promoted to adult_enabled: %v",
						targetHandle, err)
					w.Write([]byte(`<span style="color:#ef4444;font-size:12px">Flag set, but content setting was not promoted — check logs.</span>`))
					return
				}
			}
		}
