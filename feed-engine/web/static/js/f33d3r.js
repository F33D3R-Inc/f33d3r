...
(function() {
  ...
  function bind(es) {
    ...
    es.addEventListener('follow_accepted', function(e) {
      window._onSSEFollowAccepted && window._onSSEFollowAccepted(e);
    });
...
window._onSSEFollowAccepted = function(e) {
  if (!e.data) return;
  var pial = e.data.trim();
  if (!pial) return;
  document.querySelectorAll('[data-pial="' + pial + '"]').forEach(function(old) {
    // post-follow-btn and focus-follow-dot vanish on follow (no unfollow from feed card)
    if (old.classList.contains('post-follow-btn') || old.classList.contains('focus-follow-dot')) {
      old.remove();
      return;
    }
    var cls = old.classList.contains('suggested-follow-btn') ? 'suggested-follow-btn'
            : old.classList.contains('profile-follow-btn')   ? 'profile-follow-btn'
            : old.classList.contains('pdf-follow-btn')       ? 'pdf-follow-btn'
            : null;
    if (!cls) return;
    // Already in following state — skip to avoid redundant DOM churn
    if (old.classList.contains('following') || old.classList.contains('pdf-follow-btn--following')) return;
    var extraCls = cls === 'pdf-follow-btn' ? ' pdf-follow-btn--following' : ' following';
    var source = cls === 'suggested-follow-btn' ? 'suggested'
               : cls === 'pdf-follow-btn'       ? 'post_detail'
               : '';
    var sourceJson = source ? ',"source":"' + source + '"' : '';
    var wrap = document.createElement('div');
    wrap.innerHTML = '<button class="' + cls + extraCls + '" data-pial="' + pial +
      '" hx-post="/events" hx-target="this" hx-swap="outerHTML"' +
      ' hx-vals=\'{"event_type":"unfollow","target_pial":"' + pial + '"' + sourceJson + '}\'>Following</button>';
    var neo = wrap.firstElementChild;
    old.replaceWith(neo);
    if (typeof htmx !== 'undefined') htmx.process(neo);
  });
};
...
window.updateCharCount = function() {
  ...
  if (inner) {
    // Show remaining count when > 20000 chars typed (remaining < 5000)
    var show = used > 20000;
...
// ── Feed surface switcher ─────────────────────────────────────────────────────
// Switches the active feed tab and re-loads the feed container via HTMX.
// Called by onclick on every feed-tab button.
window.switchSurface = function(btn, surface) {
  document.querySelectorAll('.feed-tab').forEach(function(t) { t.classList.remove('is-active'); });
  btn.classList.add('is-active');
  var fc = document.getElementById('feed-container');
  if (!fc) return;
  var url = '/facets/works/feed?surface=' + encodeURIComponent(surface);
  fc.setAttribute('hx-get', url);
  htmx.process(fc);
  htmx.trigger(fc, 'load');
  // Update browser URL so back button restores the correct surface
  history.pushState({surface: surface}, '', '/?surface=' + encodeURIComponent(surface));
};

// Restore feed surface tab when browser back/forward navigates between surface URLs
window.addEventListener('popstate', function(e) {
  var surface = (e.state && e.state.surface)
    || new URLSearchParams(window.location.search).get('surface')
    || 'following';
  var tab = document.querySelector('.feed-tab[data-surface="' + surface + '"]');
  var fc  = document.getElementById('feed-container');
  if (!tab || !fc) return;
  document.querySelectorAll('.feed-tab, .tab').forEach(function(t) { t.classList.remove('is-active'); });
  tab.classList.add('is-active');
  htmx.ajax('GET', '/facets/works/feed?surface=' + encodeURIComponent(surface),
    { target: fc, swap: 'innerHTML' });
});

// ── Profile tab switcher ──────────────────────────────────────────────────────
// Switches the active profile tab and re-loads the profile feed container.
window.switchProfileTab = function(btn, surface, handle) {
  document.querySelectorAll('.profile-tab').forEach(function(t) { t.classList.remove('active'); });
  btn.classList.add('active');
  var pf = document.getElementById('profile-feed');
  if (!pf) return;
  var url = '/facets/works/feed?surface=' + encodeURIComponent(surface) + '&handle=' + encodeURIComponent(handle);
  pf.setAttribute('hx-get', url);
  htmx.process(pf);
  htmx.trigger(pf, 'load');
};
...
document.addEventListener('DOMContentLoaded', function() { formatPostTimes(); hydrateVoiceBars(); formatVisionRings(); });
document.addEventListener('htmx:afterSwap', function(e) { formatPostTimes(e.detail.target); hydrateVoiceBars(e.detail.target); formatVisionRings(e.detail.target); });

// formatVisionRings — sets SVG arc stroke-dashoffset based on expiry time remaining.
// Called on DOMContentLoaded and after every HTMX swap.
// Circumference = 2π * r19 ≈ 119.38. Full ring = 0 offset; empty = full circumference.
window.formatVisionRings = function(root) {
  var scope = root || document;
  scope.querySelectorAll && scope.querySelectorAll('.vision-ring__arc[data-expires]').forEach(function(arc) {
    var expires = parseInt(arc.dataset.expires, 10) * 1000;
    var remaining = Math.max(0, expires - Date.now());
    var fraction = Math.min(1, remaining / (24 * 3600 * 1000));
    var c = 119.38;
    arc.style.strokeDasharray = c;
    arc.style.strokeDashoffset = (c * (1 - fraction)).toFixed(2);
  });
};
...
window.openTipModal = function(handle) {
  ...
  var label = document.getElementById('tip-modal-handle');
  var input = document.getElementById('tip-amount-input');
  var extSlot = document.getElementById('tip-external-links');
  if (!modal) return;
  if (label) label.textContent = '@' + handle;
  if (input) input.value = '';
  if (extSlot) extSlot.innerHTML = '';
  modal.classList.remove('hidden');
  document.body.style.overflow = 'hidden';
  if (input) input.focus();
  // Load external payment links for this user (Lane 2 read)
  if (extSlot && handle) {
    fetch('/facets/tip_external_links?handle=' + encodeURIComponent(handle))
      .then(function(r) { return r.ok ? r.text() : ''; })
      .then(function(html) { if (extSlot) extSlot.innerHTML = html; })
      .catch(function() {});
  }
};
...
window.submitTip = function() {
  var input = document.getElementById('tip-amount-input');
  var btn   = document.getElementById('tip-send-btn');
  var amount = parseFloat(input ? input.value : 0);
  if (!_tipHandle || isNaN(amount) || amount <= 0) {
    if (typeof toast === 'function') toast('Enter a valid amount', 'error');
    return;
  }
  if (btn) { btn.disabled = true; btn.textContent = 'Sending…'; }
  var fd = new FormData();
  fd.append('event_type', 'tip');
  fd.append('target_handle', _tipHandle);
  fd.append('amount_aet', String(Math.round(amount * 100)));
  fetch('/events', { method: 'POST', body: fd })
    .then(function(r) {
      return r.json().then(function(data) {
        if (r.ok && data.ok !== false) {
          if (typeof toast === 'function') toast('Tip sent to @' + _tipHandle + ' ✓');
          window.closeTipModal();
        } else {
          var msg = (data && data.error) ? data.error : 'Tip failed. Please try again.';
...
window.submitCreateList = function() {
  ...
  var desc = (document.getElementById('list-desc-input').value || '').trim();
  var priv = document.getElementById('list-private-toggle').checked;
  var fd = new FormData();
  fd.append('event_type', 'create_list');
  fd.append('name', name);
  fd.append('description', desc);
  fd.append('is_public', priv ? 'false' : 'true');
  fetch('/events', { method: 'POST', body: fd })
    .then(function(r) {
    ...
    .catch(function() { toast('Could not create list', 'error'); });
};
window.openAddMember = function(listId) {
  document.getElementById('add-member-list-id').value = listId;
  document.getElementById('add-member-modal').classList.remove('hidden');
  document.getElementById('add-member-handle').focus();
};
window.submitAddMember = function() {
  var handle = (document.getElementById('add-member-handle').value || '').replace(/^@/, '').trim();
  var listId = document.getElementById('add-member-list-id').value;
  if (!handle) { toast('Enter a handle', 'error'); return; }
  var fd = new FormData();
  fd.append('event_type', 'list_add_member');
  fd.append('list_id', listId);
  fd.append('handle', handle);
  fetch('/events', { method: 'POST', body: fd })
    .then(function(r) {
      if (r.ok) { toast('@' + handle + ' added to list'); document.getElementById('add-member-modal').classList.add('hidden'); }
      else { r.text().then(function(t) { toast(t || 'Could not add member', 'error'); }); }
    });
};
