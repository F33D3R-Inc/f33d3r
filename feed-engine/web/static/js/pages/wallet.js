// wallet.js — balance, transactions, send sheet and governance.
// Registered as a Shell page: executed once per session, the mount runs on
// every arrival of the wallet Playground. Document-level listeners are
// installed once.
F33D3R.page('wallet', function () {
  ...
  window.openSendSheet        = openSendSheet;
  window.closeSendSheet       = closeSendSheet;
  window.onSendRecipientInput = onSendRecipientInput;
  window.selectSendRecipient  = selectSendRecipient;
  window.sendTip              = sendTip;
  window.loadProposals        = loadProposals;
  window.openProposal         = openProposal;
  window.castVote             = castVote;
  window.toggleNewProposal    = toggleNewProposal;
  window.submitProposal       = submitProposal;

  // ── Document-level listeners — installed once ─────────────────────────────────
  F33D3R.once('wallet:listeners', function () {
    ...
    document.body.addEventListener('htmx:afterSettle', function() {
      if (!document.querySelector('meta[name="f33d3r:pial"]')?.content) return;
      if (document.getElementById('tx-list') && window.loadTxHistory) window.loadTxHistory();
      if (document.getElementById('proposals-list') && window.loadProposals) window.loadProposals();
    });
    // Escape closes the sheet
    document.addEventListener('keydown', function(e) {
      if (e.key === 'Escape' && window.closeSendSheet) window.closeSendSheet();
    });
  ...
  // ── Boot ──────────────────────────────────────────────────────────────────────
  if (MY_PIAL) {
    Promise.all([loadBalance(), loadTxHistory(), loadProposals()]);
  }
