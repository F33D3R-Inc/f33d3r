// kyc.js — identity verification flow. Registered as a Shell page: executed
// once per session, the mount runs on every arrival of the /kyc Playground and
// its teardown releases any camera stream the flow still holds.
F33D3R.page('kyc', function (root) {
  ...
  async function _kycSubmitVerification() {
    ...
    try {
      ...
      if (isCreatorMode && isSuccess) {
        ...
        // Append a status link on success — a Playground navigation like any anchor.
  ...
  window.kycGo              = kycGo;
  window.kycCaptureDoc      = kycCaptureDoc;
  window.kycDocFromFile     = kycDocFromFile;
  window.kycConfirmDoc      = kycConfirmDoc;
  window.kycAgeSubmit       = kycAgeSubmit;
  window.kycAgreementToggle = kycAgreementToggle;

  // ── Init ───────────────────────────────────────────────────────────────────────
  var flow = root.querySelector ? root.querySelector('#kyc-flow-root') : document.getElementById('kyc-flow-root');
  if (flow) {
    isCreatorMode = flow.dataset && flow.dataset.kycMode === 'creator';
    kycGo('intro');
  }

  return function teardown() { stopStreams(); };
});
