...
// Runs whenever the post detail Playground arrives. Wires two SSE callbacks:
...
// for this post to the current user's SSE session. Leaving the page (the Shell
// tears the mount down before the next Playground swaps in) unregisters it.
F33D3R.page('post', function (root) {
  'use strict';

  var postEl = root.querySelector('[data-watch-post]');
  if (!postEl) return;
  ...
  function unwatch() { registerWatch('unwatch'); }

  // Register immediately; unregister when the document goes away.
  registerWatch('watch');
  ...
  return function teardown() {
    unwatch();
    window.removeEventListener('pagehide', unwatch);
    window._onSSENewReply = null;
  };
});
