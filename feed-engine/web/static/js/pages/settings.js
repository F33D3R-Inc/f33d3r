...
//
// Registered as a Shell page: executed once per session, the mount wires the
// settings Playground on every arrival. Document-level listeners are installed
// once.

F33D3R.page('settings', function () {
  ...
  function _initSettingsMobileNav() {
    ...
    F33D3R.once('settings:panel-swap', function () {
    ...
    // On arrival with a section in the URL, open the panel on mobile
    if (window.location.pathname.match(/^\/settings\/.+/)) {
      _settingsOpenPanel();
    }
  }

  window.initCrop            = initCrop;
  window.closeCropModal      = closeCropModal;
  window.applyCrop           = applyCrop;
  window.onPronounsChange    = onPronounsChange;
  window.detectLocation      = detectLocation;
  window.settingsNavActivate = settingsNavActivate;
