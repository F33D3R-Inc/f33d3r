...
pub struct NexusState {
    pub pool: PgPool,
    /// Same service-to-service secret held by `AppState`. The NEXUS mutations —
    /// identity creation, persona linking, AML aggregation — are all invoked by
    /// feed-engine (server-side, after authenticating the user) or by ain-soph,
    /// never by a browser directly, so they require it too.
    pub internal_api_key: InternalApiKey,
}

impl FromRef<Arc<NexusState>> for InternalApiKey {
    fn from_ref(state: &Arc<NexusState>) -> InternalApiKey {
...
async fn verify(
    ...
    if req.confidence < 0.85 {
        return Err(err(
            ...
            "biometric confidence below threshold",
        ));
    }
    if req.biometric_hash.is_empty() || req.document_hash.is_empty() || req.pial_shard_id.is_empty()
    {
        return Err(err(
            ...
            "biometric_hash, document_hash, pial_shard_id required",
        ));
    }
...
async fn link_confirm(
    ...
    let (nexus_id, target_shard) =
        row.ok_or_else(|| err(StatusCode::NOT_FOUND, "request not found or expired"))?;
    ...
    if req.pial_shard_id != target_shard {
        return Err(err(
            ...
            "only the target account can confirm this link",
        ));
    }
...
async fn link_initiate(
    ...
    if req.source_pial_shard_id.is_empty() || req.target_pial_shard_id.is_empty() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "source_pial_shard_id and target_pial_shard_id required",
        ));
    }
    ...
    let target_nexus: Option<Uuid> =
        sqlx::query_scalar("SELECT nexus_id FROM nexus_persona_links WHERE pial_shard_id = $1")
    ...
    if target_nexus.is_some() {
        return Err(err(
            StatusCode::CONFLICT,
            "target account is already linked to a nexus",
        ));
    }
    ...
    Ok(Json(
        json!({ "result": "pending", "request_id": request_id }),
    ))
}
...
async fn list_personas(
    ...
    let nexus_id =
        nexus_id.ok_or_else(|| err(StatusCode::NOT_FOUND, "shard not linked to any nexus"))?;
    ...
    let personas: Vec<Value> = rows
        .iter()
        .map(|(shard_id, ptype, label, primary)| {
            json!({
    ...
    let tier: i16 =
        sqlx::query_scalar("SELECT verity_tier FROM nexus_identities WHERE nexus_id = $1")
            .bind(nexus_id)
            .fetch_one(&state.pool)
            .await
            .unwrap_or(1);
...
async fn unlink_persona(
    ...
    let nexus_id =
        caller_nexus.ok_or_else(|| err(StatusCode::FORBIDDEN, "caller not linked to any nexus"))?;
    ...
    if is_primary {
        return Err(err(
            StatusCode::FORBIDDEN,
            "cannot unlink the primary persona",
        ));
    }
    ...
    if deleted == 0 {
        return Err(err(
            ...
            "persona not found or not unlinkable",
        ));
    }
...
async fn aml_get(
    ...
    let (nexus_id, total, flagged) =
        row.ok_or_else(|| err(StatusCode::NOT_FOUND, "shard not linked to any nexus"))?;
...
async fn aml_record(
    ...
    // Called by ain-soph after a withdrawal is approved. The AML aggregate is a
    // compliance record; only the settling brain may write it.
    caller.require_service()?;
...
#[cfg(test)]
mod auth_pins {
    /// Structural pin: every mutating NEXUS handler takes the `Caller` extractor
    /// and enforces `require_service`. These routes create identities, link
    /// personas and write AML aggregates — all invoked by feed-engine or ain-soph
    /// server-side, never by a browser — so a new one that forgets the gate must
    /// fail here rather than accept unauthenticated writes in production.
    #[test]
    fn every_mutating_nexus_handler_requires_a_service_caller() {
        let src = include_str!("nexus.rs");
        for handler in [
            "async fn verify(",
            "async fn link_confirm(",
            "async fn link_decline(",
            ...
            let sig_end = src[start..]
                ...
                + start;
            assert!(
                src[start..sig_end].contains("caller: Caller"),
                "{handler} does not take the Caller extractor — it would accept \
                 unauthenticated requests."
            );
            let next = src[sig_end..]
                .find("\nasync fn ")
                .map(|i| sig_end + i)
                .unwrap_or(src.len());
            assert!(
                src[sig_end..next].contains("caller.require_service()"),
