...
fn ladder_for(source_height: u32) -> Vec<Variant> {
    let mut out: Vec<Variant> = VARIANTS.iter()
...
async fn transcode_variant(
    ...
    if nvenc_enabled() {
        cmd.args(["-c:v", "h264_nvenc", "-preset", "p4", "-profile:v", "high", "-pix_fmt", "yuv420p"]);
    } else {
        cmd.args(["-c:v", "libx264", "-preset", "veryfast", "-profile:v", "main", "-level", "4.1", "-pix_fmt", "yuv420p"]);
    }
    cmd.args(["-vf", &scale, "-b:v", &video_kbps, "-maxrate", &video_max, "-bufsize", &video_buf]);
    cmd.args(["-g", "48", "-keyint_min", "48"]);
    if !nvenc_enabled() { cmd.args(["-sc_threshold", "0"]); }
    cmd.args(["-c:a", "aac", "-b:a", &audio_kbps, "-ac", "2", "-ar", "48000"]);
    cmd.args(["-f", "hls", "-hls_time", "4", "-hls_list_size", "0", "-hls_playlist_type", "vod", "-hls_segment_filename"]);
    cmd.arg(&segment_filename).arg(&playlist);
    let out = cmd.stdout(Stdio::null()).stderr(Stdio::piped()).output().await
...
async fn burn_watermarked_mp4(
    ...
    if safe_handle.is_empty() {
        ...
        cmd.args(["-maxrate", &format!("{}k", top_variant.video_kbps + top_variant.video_kbps / 8)]);
        cmd.args(["-bufsize", &format!("{}k", top_variant.video_kbps * 2)]);
        cmd.args(["-c:a", "aac", "-b:a", &format!("{}k", top_variant.audio_kbps), "-ac", "2", "-ar", "48000"]);
        cmd.args(["-movflags", "+faststart", "-f", "mp4"]).arg(dest);
        let out = cmd.stdout(Stdio::null()).stderr(Stdio::piped()).output().await
            .context("ffmpeg watermarked mp4 (no handle) failed to spawn")?;
        if !out.status.success() {
            bail!("ffmpeg watermarked mp4 exit {}: {}", out.status, String::from_utf8_lossy(&out.stderr));
        }
        return Ok(());
    }
...
pub async fn run(
    ...
    run_clean_pass(source, &clean_dir, &ladder, width, height).await
    ...
    let top_variant = *ladder.last().expect("ladder always has at least one variant");
    burn_watermarked_mp4(source, top_variant, creator_handle, &wm_mp4_path).await
    ...
    let poster_url = if poster_ok { format!("{clean_base}/poster.jpg") } else { String::new() };
    let variants_out = ladder.iter().map(|v| VariantOutput {
        name: v.name.into(),
        height: v.height,
