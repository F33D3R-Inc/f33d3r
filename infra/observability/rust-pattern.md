...
pub async fn middleware(req: Request<Body>, next: Next) -> Response<Body> {
    ...
    // The route PATTERN that matched, never the request path. Metric labels
    // must draw from a set fixed at compile time — the route table — because
    // Prometheus keeps one time series per distinct value for the life of the
    // process, and a request path is unbounded (every name, address or Number
    // ever resolved is a new one). A request that matched no route gets one
    // shared label, or a scanner walking a wordlist mints a series per word.
    // Requires the middleware to be mounted with `Router::layer` after every
    // route and merge, which is what makes `MatchedPath` available. The raw
    // path goes to the log line, where a distinct value costs nothing.
