...
# Similarity thresholds. The audio cuts are scanner.AUDIO_DUPLICATE_* — one
# definition, imported, because a sweep that scores differently from the live
# path would rewrite lineage the live path never would have.
...
# ── similarity helpers ────────────────────────────────────────────────────────
# Audio similarity is scanner.AudioFingerprintComparer — the same code the live
# service runs, not a copy of it.
...
def _closest_audio_match(sqlite_conn: sqlite3.Connection, audio_fp: str) -> Optional[dict]:
    """
    Best-scoring stored fingerprint for this audio, or None.
    Returns {"score", "row", "verdict"} with verdict "block" or "review".

    An undecodable fingerprint is reported against the row that holds it and
    skipped. It is never scored as zero — a sweep that silently scores zero is
    how the previous implementation stayed broken for its entire life.
    """
    try:
        comparer = scanner.AudioFingerprintComparer(audio_fp)
    except scanner.AudioFingerprintError as e:
        logger.error("audio dedup skipped — query fingerprint will not decode: %s", e)
        return None

    if not comparer.usable:
        logger.info("audio dedup skipped for this asset — %s", comparer.reason)
        return None
    ...
    for row in candidates:
        try:
            ...
            logger.error("stored fingerprint for asset=%s will not decode: %s",
                         row["asset_id"], e)
            continue
        if score is None:
            continue
...
def _find_duplicate(sqlite_conn: sqlite3.Connection, sha256: str, audio_fp: str, phash: str) -> tuple[Optional[dict], Optional[str]]:
    ...
    Only a block-grade audio match is returned. A review-band match is logged
    and left alone, because this sweep rewrites lineage and a wrong rewrite
    takes a work away from the creator who made it.
    """
    ...
    if audio_fp:
        match = _closest_audio_match(sqlite_conn, audio_fp)
        if match and match["verdict"] == "block":
            return match["row"], "audio"
        if match:
            logger.warning(
                "audio %.4f similar to @%s's asset %s — below the %.2f block cut, "
                "reported not acted on",
                match["score"], match["row"].get("uploader_handle", ""),
                match["row"].get("asset_id", ""),
                scanner.AUDIO_DUPLICATE_BLOCK_SCORE,
            )
