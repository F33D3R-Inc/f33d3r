...
# libretranslate) are intentionally NOT in BUILD_ORDER — they have no build:
# config and `compose build` on them warns "No services to build". They are
# started by `compose up -d` like everything else.
...
# typo — fail loudly instead of silently doing a full 23-service rebuild.
if [[ -n "${1:-}" && "${1}" != "--all" ]]; then
...
# This rebuilds ALL ~23 services and is SLOW — confirm it was intentional.
