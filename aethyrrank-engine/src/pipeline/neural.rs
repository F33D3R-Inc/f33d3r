...
fn score_item(user: &UserState, item: &ContentItem) -> NeuralScore {
    ...
    let quality = if eng.impressions > 0.0 {
        ...
        engagement_rate_score(raw / eng.impressions)
    } else {
        0.0
    };
...
mod tests {
    ...
    fn score_in_unit_interval() {
        ...
        let item = make_item(vec![0.5; 8], 1000.0);
    ...
    fn aligned_user_scores_higher() {
        let mut aligned = vec![0.0_f32; 8];
        aligned[0] = 1.0;
        let mut opposite = vec![0.0_f32; 8];
        opposite[7] = 1.0;
        let user_aligned = make_user(aligned.clone());
        let user_opposite = make_user(opposite);
        let item = make_item(aligned, 500.0);
    ...
    fn batch_returns_one_score_per_item() {
        let user = make_user(vec![0.5; 8]);
        let item1 = make_item(vec![0.5; 8], 100.0);
        let item2 = make_item(vec![0.2; 8], 200.0);
