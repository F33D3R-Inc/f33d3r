...
if [ ! -f .env.prod ]; then
  ...
  err "  nano .env.prod"
  err ""
  err "Required changes (every CHANGE_ME in the file):"
  err "  POSTGRES_PASSWORD       — set a strong password"
  err "  SESSION_SECRET          — run: openssl rand -base64 48"
  err "  INTERNAL_API_KEY        — run: openssl rand -hex 32"
  err "  MEDIA_TOKEN_SECRET      — run: openssl rand -base64 48"
  err "  METRICS_TOKEN           — run: openssl rand -hex 24"
  err "  THEMIS_ECDH_PRIVKEY_B64 — run: openssl ecparam -name prime256v1 -genkey -noout | openssl pkcs8 -topk8 -nocrypt -outform DER | base64 -w0"
  err ""
  err "provision-server.sh generates all of these for a fresh server."
...
if grep -q "CHANGE_ME" .env.prod; then
  err ".env.prod still contains placeholder values:"
  grep -n "CHANGE_ME" .env.prod | sed 's/=.*//' | while read -r line; do err "  $line"; done
  err "Edit it before deploying."
  exit 1
fi
INTERNAL_KEY_LEN=$(grep -E '^INTERNAL_API_KEY=' .env.prod | head -n1 | cut -d= -f2- | tr -d '[:space:]' | wc -c)
if [ "$INTERNAL_KEY_LEN" -lt 32 ]; then
  err "INTERNAL_API_KEY in .env.prod is missing or shorter than 32 characters."
