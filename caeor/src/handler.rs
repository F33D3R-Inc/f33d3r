...
fn process_avatar(state: &AppState, img: DynamicImage) -> Result<Json<UploadResponse>, (StatusCode, String)> {
    ...
    let primary = urls.iter().find(|u| u.variant == "feed")
        .map(|u| u.url.clone())
        .unwrap_or_default();
...
fn process_header(state: &AppState, img: DynamicImage) -> Result<Json<UploadResponse>, (StatusCode, String)> {
    let id = Uuid::new_v4().to_string();
    ...
    let primary = urls.iter().find(|u| u.variant == "md")
        .map(|u| u.url.clone())
        .unwrap_or_default();
...
fn process_post(state: &AppState, img: DynamicImage) -> Result<Json<UploadResponse>, (StatusCode, String)> {
    let id = Uuid::new_v4().to_string();
    ...
    urls.push(Derivative { variant: "thumb".into(), url: format!("/static/media/posts/{thumb_fname}"), width: 300, height: 300 });
    ...
    urls.push(Derivative { variant: "feed".into(), url: format!("/static/media/posts/{feed_fname}"), width: fw, height: fh });
    ...
    urls.push(Derivative { variant: "full".into(), url: format!("/static/media/posts/{full_fname}"), width: fw2, height: fh2 });
...
fn store_video(state: &AppState, data: &[u8], ext: &str) -> Result<Json<UploadResponse>, (StatusCode, String)> {
    let dir = PathBuf::from(&state.media_dir).join("posts");
