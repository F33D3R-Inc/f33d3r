...
pub async fn record_and_check(
    ...
    let resp = match http.post(&url).json(&body).send().await {
        ...
        Err(e) => {
            ...
            return AmlCheckResult { period_total_units: 0, flag_for_edd: false };
        }
    };
    ...
    let period_total = body["total_withdrawn_uaet"].as_i64().unwrap_or(withdrawal_units);
    let flag         = body["flag_for_edd"].as_bool().unwrap_or(period_total >= NEXUS_AML_THRESHOLD_UNITS);

    AmlCheckResult { period_total_units: period_total, flag_for_edd: flag }
}
