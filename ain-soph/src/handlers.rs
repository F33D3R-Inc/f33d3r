...
const FEE_ACCOUNT: &str = "ain_soph_fees";
...
pub async fn get_supply(
...
) -> Result<Json<SupplyResponse>, WalletError> {
    ...
    let fee_pool: (i64,) = sqlx::query_as(
        "SELECT COALESCE(balance, 0) FROM accounts WHERE user_id = $1"
    )
    .bind(FEE_ACCOUNT)
...
pub async fn get_balance(
    ...
    // Auto-create account on first lookup — balance starts at 0, network shows Online.
    let mut tx = pool.begin().await.map_err(WalletError::Db)?;
    db::ensure_account(&mut tx, &user_id).await?;
    tx.commit().await.map_err(WalletError::Db)?;

    let balance = db::get_balance(&pool, &user_id).await?
        .unwrap_or(0);

    let updated_at = db::get_account_updated_at(&pool, &user_id).await?
...
pub async fn list_transactions(
    ...
    Ok(Json(TxHistoryResponse { user_id, entries, count }))
}
...
pub async fn deposit(
    ...
    if req.amount_cents <= 0 { return Err(WalletError::InvalidAmount); }
    ...
    .bind(&req.user_id).bind(req.amount_cents).bind(fee).bind(net)
    ...
    db::ensure_account(&mut tx, &req.user_id).await?;
    db::ensure_account(&mut tx, FEE_ACCOUNT).await?;
    db::credit_account(&mut tx, &req.user_id, net).await?;
    db::credit_account(&mut tx, FEE_ACCOUNT, fee).await?;
    db::append_ledger(&mut tx, tx_id, &req.user_id, "credit", net).await?;
    if fee > 0 {
        db::append_ledger(&mut tx, tx_id, FEE_ACCOUNT, "credit", fee).await?;
    }

    tx.commit().await?;
    info!(user_id = %req.user_id, amount = req.amount_cents, fee, net, "AET deposit completed");
...
pub async fn transfer(
    ...
    if req.amount_cents <= 0  { return Err(WalletError::InvalidAmount); }
    if req.from_user_id == req.to_user_id { return Err(WalletError::SelfTransfer); }
    ...
    .bind(tx_id).bind(&req.idempotency_key)
    .bind(&req.from_user_id).bind(&req.to_user_id)
    ...
    db::ensure_account(&mut tx, &req.from_user_id).await?;
    db::ensure_account(&mut tx, &req.to_user_id).await?;
    db::ensure_account(&mut tx, FEE_ACCOUNT).await?;

    let sender_balance = db::locked_balance(&mut tx, &req.from_user_id).await?;
    if sender_balance < req.amount_cents {
        tx.rollback().await?;
        return Err(WalletError::InsufficientBalance);
    }

    db::debit_account(&mut tx, &req.from_user_id, req.amount_cents).await?;
    db::credit_account(&mut tx, &req.to_user_id, net).await?;
    db::credit_account(&mut tx, FEE_ACCOUNT, fee).await?;
    db::append_ledger(&mut tx, tx_id, &req.from_user_id, "debit",  req.amount_cents).await?;
    db::append_ledger(&mut tx, tx_id, &req.to_user_id,   "credit", net).await?;
    if fee > 0 {
        db::append_ledger(&mut tx, tx_id, FEE_ACCOUNT, "credit", fee).await?;
    }
...
pub async fn tip(
    ...
    if req.amount_aet <= 0.0 { return Err(WalletError::InvalidAmount); }
    if req.from_pial_id == req.to_pial_id { return Err(WalletError::SelfTransfer); }

    // Convert AET → storage units
    let amount_units = (req.amount_aet * AET_DECIMALS as f64).round() as i64;
    if amount_units <= 0 { return Err(WalletError::InvalidAmount); }
    ...
    .bind(&req.from_pial_id).bind(&req.to_pial_id)
    ...
    db::ensure_account(&mut tx, &req.from_pial_id).await?;
    db::ensure_account(&mut tx, &req.to_pial_id).await?;
    db::ensure_account(&mut tx, FEE_ACCOUNT).await?;

    let sender_balance = db::locked_balance(&mut tx, &req.from_pial_id).await?;
    if sender_balance < amount_units {
        tx.rollback().await?;
        return Err(WalletError::InsufficientBalance);
    }

    db::debit_account(&mut tx, &req.from_pial_id, amount_units).await?;
    db::credit_account(&mut tx, &req.to_pial_id, net).await?;
    db::credit_account(&mut tx, FEE_ACCOUNT, fee).await?;
    db::append_ledger(&mut tx, tx_id, &req.from_pial_id, "debit",  amount_units).await?;
    db::append_ledger(&mut tx, tx_id, &req.to_pial_id,   "credit", net).await?;
    if fee > 0 {
        db::append_ledger(&mut tx, tx_id, FEE_ACCOUNT, "credit", fee).await?;
    }
...
pub async fn withdraw(
    ...
    let balance = db::locked_balance(&mut tx, &req.user_id).await?;
    if balance < req.amount_cents {
        tx.rollback().await?;
        return Err(WalletError::InsufficientBalance);
    }

    db::debit_account(&mut tx, &req.user_id, req.amount_cents).await?;
    db::credit_account(&mut tx, FEE_ACCOUNT, fee).await?;
    db::append_ledger(&mut tx, tx_id, &req.user_id,  "debit",  req.amount_cents).await?;
    if fee > 0 {
        db::append_ledger(&mut tx, tx_id, FEE_ACCOUNT, "credit", fee).await?;
    }

    tx.commit().await?;
    info!(user_id = %req.user_id, amount = req.amount_cents, fee, net, "AET withdrawal completed");
...
pub async fn create_proposal(
    ...
    if req.options.len() < 2 {
        return Err(WalletError::BadRequest("at least 2 options required".into()));
    }
    if !["token","identity","equal"].contains(&req.weight_model.as_str()) {
        return Err(WalletError::BadRequest("weight_model must be token|identity|equal".into()));
    }
    if req.ends_at <= chrono::Utc::now() {
        return Err(WalletError::BadRequest("ends_at must be in the future".into()));
    }
    ...
    .bind(&req.creator_id).bind(&req.weight_model)
    .bind(&req.options).bind(req.ends_at)
    .execute(&pool).await?;

    info!(id = %id, title = %req.title, creator = %req.creator_id, "proposal created");
    ...
    for row in rows {
        let total_votes: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM votes WHERE proposal_id = $1")
                .bind(row.id).fetch_one(&pool).await?;
...
pub async fn get_proposal(
    ...
    let results: Vec<OptionResult> = row.options.iter().map(|opt| {
        ...
        OptionResult { option: opt.clone(), vote_count: count, total_weight: weight, pct }
    }).collect();

    let leading = results.iter().max_by(|a, b| a.total_weight.cmp(&b.total_weight))
    ...
    Ok(Json(ProposalDetailResponse { proposal: summary, results, total_weight, leading }))
}
...
pub async fn cast_vote(
    ...
    let weight: i64 = match row.weight_model.as_str() {
        "token" => {
            ...
            db::get_balance(&pool, &req.voter_id).await?.unwrap_or(0).max(1)
        }
        "identity" | _ => 1,
    };
    ...
    .bind(proposal_id).bind(&req.voter_id).bind(&req.option).bind(weight)
    .execute(&pool).await;

    match result {
        Ok(_) => {}
        Err(sqlx::Error::Database(e)) if e.constraint() == Some("votes_proposal_id_voter_id_key") => {
    ...
    info!(
        proposal = %proposal_id, voter = %req.voter_id,
