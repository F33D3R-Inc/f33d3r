...
/// Both parties arrive as NAMES, not as rows from another brain.
///
/// A caller may send a bare PIAL uuid (the historic wire form, still accepted),
/// `pial:<uuid>`, `@handle`, or `handle:<h>`. Every form is reduced to exactly
/// one PIAL through Manhattan before anything is written — see `identity`. That
/// is what stops a report filed against a handle from landing on whoever held
/// that handle last.
#[derive(Debug, Deserialize)]
pub struct CreateReportRequest {
    #[serde(default, alias = "reporter")]
    pub reporter_pial_id: Option<String>,
    #[serde(default, alias = "reported")]
    pub reported_pial_id: Option<String>,
    pub content_id: String,
