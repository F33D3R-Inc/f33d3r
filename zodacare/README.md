...
3. If risk score exceeds threshold → POST /v1/user/:ident/action (warn/restrict/ban)
...
`:ident` is a NAME, resolved through Manhattan — a bare PIAL uuid, `pial:<uuid>`,
`@handle` or `handle:<h>`. It is never a stored handle copy, so a handle that has
been transferred or revoked cannot carry a ban to the wrong person.

| Method | Path                     | Description             |
|--------|--------------------------|-------------------------|
| `GET`  | `/v1/user/:ident/risk`   | Get risk score (0–100)  |
| `POST` | `/v1/user/:ident/action` | Apply moderation action |
...
| `GET`  | `/health`   | Service health, `manhattan`, outbox backlog  |
...
All actions are forwarded to Elohim Veni, which writes the capability change to the PIAL record (the authority) and keeps an immutable audit log. The PIAL forwarded is the one Manhattan resolved, so the local record and the enforcement always name the same person.

---

## Identity

Zodacare does not own identity — `elohim-veni` does. Every person a report, risk
profile or ban names is a REFERENCE, resolved through Manhattan rather than
joined out of another brain's tables.

- `MANHATTAN_URL` — the naming plane. Unset means handle lookups are refused
  (503) rather than guessed; PIAL-keyed calls still work and registrations still
  queue.
- Registrations are durable. Database triggers on `content_reports`,
  `moderation_actions` and `user_risk_profiles` enqueue an identity-node
  registration into `manhattan_outbox` inside the SAME transaction as the row
  that caused it, and a background drain delivers them in order with backoff.
  `/health` reports the undelivered backlog.
...
│   ├── db.rs               ← Report storage, risk scoring, Manhattan outbox schema
│   ├── identity.rs         ← Name → PIAL resolution; refuses rather than guesses
│   ├── manhattan_client.rs ← Shared naming-plane client (copy of infra/manhattan)
│   ├── outbox.rs           ← Durable registration drain
