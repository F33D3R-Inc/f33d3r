...
/// The platform's own fee sink. It is keyed on the nil UUID precisely because no
/// PIAL is ever issued that value, so it is not a person and has no identity
/// node in the naming plane.
const TREASURY_PIAL: Uuid = Uuid::nil();

/// Resolve a PIAL through Manhattan before any row in this database is keyed on
/// it.
///
/// A ledger account, a funding event and a settlement must all denote an
/// identity the naming plane agrees exists — not one this brain inferred from a
/// caller-supplied string. Money attributed to an identity nobody can name is
/// unrecoverable in a way a wrong feed ranking is not, so the check happens
/// before the write, not after.
///
/// `ensure_node` resolves first and only registers when the name is genuinely
/// unknown, which is what makes it safe to call on every request: the common
/// path is one indexed lookup in the resolver, and this brain never mints a
/// second node for an identity elohim-veni already owns.
///
/// The three failure modes are deliberately different:
///
///   * Not configured — the naming plane is not reachable from this deployment
///     yet. Proceed, because the outbox is still recording every registration
///     transactionally and will deliver them whole once it is; the drain has
///     already said so once, loudly. This is not a licence to fall back to a
///     local join.
///   * Revoked or tombstoned — a live answer that this identity must not be
///     settled against. Refuse.
///   * Anything else — the resolver is configured and did not answer. Refuse
///     with 503. A refused transfer is retryable; a misattributed one is not,
///     and guessing is exactly what this whole plane exists to end.
async fn require_identity(state: &AppState, pial: Uuid) -> Result<(), (StatusCode, Json<Value>)> {
    if pial == TREASURY_PIAL {
        return Ok(());
    }
    match state
        .manhattan
        .ensure_node(KIND_IDENTITY, &pial_name(&pial.to_string()))
        .await
    {
        ...
        Ok(node) => Err(err(
            ...
            &format!(
                "identity {pial} is {} in the naming plane and may not be settled against",
                node.status
            ),
        )),
        Err(ManhattanError::NotConfigured) => Ok(()),
        Err(e) => Err(err(
            StatusCode::SERVICE_UNAVAILABLE,
            &format!("identity {pial} could not be resolved: {e}"),
        )),
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // The naming plane. This brain references identities by NAME and resolves
    // them here; it never reaches into elohim-veni's tables to find out who a
    // settlement belongs to.
    let manhattan = Manhattan::from_env("aethyr-ledger")?;

    // Background: deliver queued identity registrations to Manhattan, in order.
    // The triggers enqueued them in the same transaction as the money row that
    // caused them, so this loop can be as slow or as interrupted as it likes
    // without any registration being lost.
    {
        let drain_url = database_url.clone();
        let drain_manhattan = manhattan.clone();
        tokio::spawn(async move {
            manhattan_outbox::start_drain(drain_url, drain_manhattan).await;
        });
    }
...
async fn health(State(state): State<Arc<AppState>>) -> Res<Value> {
    // The backlog is read, not assumed. A health check that cannot reach the
    // database has nothing truthful to report, so it fails rather than claiming
    // "ok" — and a naming plane that has stopped accepting writes shows here as
    // a backlog that only grows.
    let backlog = manhattan_outbox::backlog(&state.pool)
        .await
        .map_err(|e| err(StatusCode::SERVICE_UNAVAILABLE, &e.to_string()))?;

    Ok(Json(json!({
        "status":                   "ok",
        "service":                  "aethyr-ledger",
...
async fn get_balance(
    ...
    // A read does not mint an account. This handler used to INSERT a
    // ledger_accounts row for whatever UUID appeared in the path, which is the
    // assumption this migration exists to remove: an account is a claim that an
    // identity exists, and only the naming plane gets to make that claim.
    let row = sqlx::query_as::<_, AccountRow>("SELECT * FROM ledger_accounts WHERE pial_id = $1")
        .bind(pial_id)
    ...
    if let Some(row) = row {
        // The account already exists, so it was created through a write path
        // that resolved the identity, and its registration is either delivered
        // or queued in the outbox. This brain's own row is authoritative for
        // this brain's own balances.
        return Ok(Json(BalanceResp {
            pial_id,
    ...
    // No local row: this ledger has never settled for this PIAL. Ask the naming
    // plane whether the identity exists at all before answering for it.
    match state
        .manhattan
        .resolve(&pial_name(&pial_id.to_string()))
        .await
    {
        // A real identity with no ledger activity holds nothing — the same
        // answer the empty row used to give, without inventing the row.
        Ok(_) | Err(ManhattanError::NotConfigured) => Ok(Json(BalanceResp {
            pial_id,
            balance_uaet: 0,
            balance_aet: 0.0,
            credit_balance: 0,
        ...
        Err(ManhattanError::NotFound) => Err(err(StatusCode::NOT_FOUND, "no such identity")),
        Err(e) => Err(err(
            StatusCode::SERVICE_UNAVAILABLE,
            &format!("identity {pial_id} could not be resolved: {e}"),
        )),
    }
}
...
async fn fund_wallet(
    ...
    require_identity(&state, req.pial_id).await?;
    db::ensure_account(&state.pool, req.pial_id)
        .await
        .map_err(internal)?;
...
async fn transfer(
    ...
    if !valid_types.contains(&req.event_type.as_str()) {
        return Err(err(
            ...
            "invalid event_type for transfer",
        ));
    }
    ...
    // Both counterparties, before either account exists and before a single
    // micro-AET moves. A settlement is a statement about two identities.
    require_identity(&state, req.from_pial).await?;
    require_identity(&state, req.to_pial).await?;

    db::ensure_account(&state.pool, req.from_pial)
        .await
        .map_err(internal)?;
    db::ensure_account(&state.pool, req.to_pial)
        .await
        .map_err(internal)?;
...
async fn relay_reward(
    ...
    require_identity(&state, req.node_pial).await?;
    db::ensure_account(&state.pool, req.node_pial)
        .await
        .map_err(internal)?;
...
async fn burn(State(state): State<Arc<AppState>>, Json(req): Json<BurnReq>) -> Res<EventResp> {
    if req.amount_uaet <= 0 {
        return Err(err(StatusCode::BAD_REQUEST, "amount must be positive"));
    }
...
async fn get_block(State(state): State<Arc<AppState>>, Path(number): Path<i64>) -> Res<BlockResp> {
    let row = sqlx::query_as::<_, BlockRow>("SELECT * FROM ledger_blocks WHERE block_number = $1")
        .bind(number)
        .fetch_optional(&state.pool)
        .await
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, &e.to_string()))?
...
async fn seal_block(pool: &PgPool) -> anyhow::Result<()> {
    ...
    let leaves: Vec<Vec<u8>> = event_ids
        .iter()
        .map(|eid| {
    ...
    info!(
        "sealed block #{block_num} — {count} events — merkle root {} — hash {}",
        &events_root[..16],
        &block_hash[..16]
    );
    ...
    let ain_soph_url =
        std::env::var("AIN_SOPH_URL").unwrap_or_else(|_| "http://ain-soph:8089".to_string());
    // A checkpoint is the platform attesting to its own sealed block — no end
    // user acts here — so Ain Soph accepts it only from an authenticated peer.
