...
func main() {
	...
	// The service-to-service secret. Verity's decision engine is service-only, so
	// the walker's direct /v1/decide probes must present it exactly as a peer
	// brain would. Same env var the brains read.
	internalKey := os.Getenv("INTERNAL_API_KEY")
	internalHdr := map[string]string{"X-Internal-Key": internalKey}
