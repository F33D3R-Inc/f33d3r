...
impl Config {
    pub fn from_env() -> Self {
        Self {
            ...
            nantar_url:        std::env::var("NANTAR_URL").unwrap_or_else(|_| "http://feed-engine:8081".into()),
            elohim_veni_url:   std::env::var("ELOHIM_VENI_URL").unwrap_or_else(|_| "http://elohim-veni:8093".into()),
            internal_api_key:  std::env::var("INTERNAL_API_KEY").unwrap_or_default(),
            mempool_ws_url:    std::env::var("MEMPOOL_WS_URL").unwrap_or_else(|_| "wss://mempool.space/api/v1/ws".into()),
            ...
            confirmation_threshold_low:  std::env::var("CONF_THRESHOLD_LOW").ok().and_then(|v| v.parse().ok()).unwrap_or(1),
            confirmation_threshold_high: std::env::var("CONF_THRESHOLD_HIGH").ok().and_then(|v| v.parse().ok()).unwrap_or(3),
            ain_soph_url:      std::env::var("AIN_SOPH_URL").unwrap_or_else(|_| "http://ain-soph:8089".into()),
