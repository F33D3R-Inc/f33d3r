...
pub async fn run_watcher(state: Arc<AppState>, watch_map: WatchMap) {
    ...
    if let Ok(pending) = db::purchases_pending(&state.pool).await {
        ...
        for p in pending {
            map.insert(p.btc_address.clone(), WatchEntry {
                purchase_id:   p.id,
...
async fn connect_watcher(state: Arc<AppState>, watch_map: WatchMap) -> Result<()> {
    info!("Connecting to mempool WebSocket: {}", state.cfg.mempool_ws_url);
...
async fn handle_payment_confirmed(
    ...
    let buyer_pubkey_b64 = match fetch_buyer_ecdh_pubkey(state, &entry.buyer_pial.to_string()).await {
        Ok(k) => k,
        Err(e) => {
    ...
    // Store in DB
    if let Err(e) = db::purchase_mark_delivered(&state.pool, entry.purchase_id, cek_for_buyer.clone()).await {
        error!("Failed to mark purchase delivered: {e}");
        return;
    }

    // Notify Nantar → SSE to buyer
    if let Err(e) = notify_nantar_delivered(state, entry, &listing.content_url, &listing.content_hash, &cek_for_buyer).await {
...
async fn fetch_buyer_ecdh_pubkey(state: &Arc<AppState>, pial_id: &str) -> Result<String> {
    let url = format!("{}/api/internal/pial/{}/ecdh-pubkey", state.cfg.nantar_url, pial_id);
...
async fn notify_nantar_delivered(
    ...
    let url = format!("{}/api/internal/themis/purchase-delivered", state.cfg.nantar_url);
    state.http.post(&url)
        .header("X-Internal-Key", &state.cfg.internal_api_key)
        .json(&serde_json::json!({
            "buyer_pial":    entry.buyer_pial.to_string(),
...
pub async fn subscribe_address(watch_map: &WatchMap, entry: WatchEntry) {
    watch_map.lock().await.insert(entry.btc_address.clone(), entry);
    ...
    if let Ok(pending) = db::purchases_pending_xrp(&state.pool).await {
        ...
        for p in pending {
            if !p.xrp_address.is_empty() {
                map.insert(p.xrp_address.clone(), XrpWatchEntry {
                    purchase_id:    p.id,
                    buyer_pial:     p.buyer_pial,
                    listing_id:     p.listing_id,
...
async fn handle_xrp_ws_message(
    state: &Arc<AppState>,
...
async fn handle_xrp_payment_confirmed(
    ...
    if let Err(e) = notify_nantar_delivered(state, btc_entry, &listing.content_url, &listing.content_hash, &cek_for_buyer).await {
...
pub async fn subscribe_xrp_address(xrp_watch_map: &XrpWatchMap, entry: XrpWatchEntry) {
    xrp_watch_map.lock().await.insert(entry.xrp_address.clone(), entry);
    // The watcher loop picks it up on next reconnect (max 5s delay).
}
